...
public class RabbitConfig {
    ...
    @Value("${app.mq.exchange}") String exchangeName;
    @Value("${app.mq.ocrQueue}") String ocrQueueName;
    @Value("${app.mq.resultQueue}") String resultQueueName;
    ...
    @Bean Queue ocrQueue()   { return QueueBuilder.durable(ocrQueueName).build(); }
    @Bean Queue resultQueue(){ return QueueBuilder.durable(resultQueueName).build(); }

    @Bean Binding bindOcr(Queue ocrQueue, TopicExchange exchange) {
        return BindingBuilder.bind(ocrQueue).to(exchange).with(ocrKey);
    }

    @Bean Binding bindResult(Queue resultQueue, TopicExchange exchange) {
